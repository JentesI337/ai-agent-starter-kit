"""Merged router for tools.*, tools.config.*, and tools.security.* control endpoints."""
...
# ---------------------------------------------------------------------------
# tools_handlers – helper functions
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# tools_handlers – public handler functions
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# tool_config_handlers – handler functions
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Builder: build_control_tools_router
# ---------------------------------------------------------------------------

def build_control_tools_router(
    *,
    tools_catalog_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    tools_profile_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    tools_policy_matrix_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    tools_policy_preview_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    ...
    context_list_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    context_detail_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    config_health_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    memory_overview_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
) -> APIRouter:
    router = APIRouter()

    @router.post("/api/control/tools.catalog")
    async def control_tools_catalog(request: JsonDict = Body(...)):
        result = (tools_catalog_handler or api_control_tools_catalog)(request)
    ...
    async def control_tools_profile(request: JsonDict = Body(...)):
        result = (tools_profile_handler or api_control_tools_profile)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result

    @router.post("/api/control/tools.policy.matrix")
    async def control_tools_policy_matrix(request: JsonDict = Body(...)):
        result = (tools_policy_matrix_handler or api_control_tools_policy_matrix)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result

    @router.post("/api/control/tools.policy.preview")
    async def control_tools_policy_preview(request: JsonDict = Body(...)):
        result = (tools_policy_preview_handler or api_control_tools_policy_preview)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
...
# ---------------------------------------------------------------------------
# Builder: build_control_tool_config_router
# ---------------------------------------------------------------------------

def build_control_tool_config_router(
    *,
    tools_config_list_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    tools_config_get_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    tools_config_update_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    tools_config_reset_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    tools_security_patterns_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    tools_security_update_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
) -> APIRouter:
    router = APIRouter()

    @router.post("/api/control/tools.config.list")
    async def control_tools_config_list(request: JsonDict = Body(...)) -> JsonDict:
        result = (tools_config_list_handler or handle_tools_config_list)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result

    @router.post("/api/control/tools.config.get")
    async def control_tools_config_get(request: JsonDict = Body(...)) -> JsonDict:
        result = (tools_config_get_handler or handle_tools_config_get)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result

    @router.post("/api/control/tools.config.update")
    async def control_tools_config_update(request: JsonDict = Body(...)) -> JsonDict:
        result = (tools_config_update_handler or handle_tools_config_update)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result

    @router.post("/api/control/tools.config.reset")
    async def control_tools_config_reset(request: JsonDict = Body(...)) -> JsonDict:
        result = (tools_config_reset_handler or handle_tools_config_reset)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result

    @router.post("/api/control/tools.security.patterns")
    async def control_tools_security_patterns(request: JsonDict = Body(...)) -> JsonDict:
        result = (tools_security_patterns_handler or handle_tools_security_patterns)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result

    @router.post("/api/control/tools.security.update")
    async def control_tools_security_update(request: JsonDict = Body(...)) -> JsonDict:
        result = (tools_security_update_handler or handle_tools_security_update)(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
