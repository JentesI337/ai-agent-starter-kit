...
def build_control_integrations_router(
    *,
    connectors_list_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    connectors_get_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    connectors_create_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    connectors_update_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    connectors_delete_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    connectors_test_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    oauth_start_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    oauth_callback_handler: Callable[[str, str], Awaitable[str]] | None = None,
    oauth_status_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
) -> APIRouter:
    router = APIRouter()

    @router.post("/api/control/integrations.connectors.list")
    async def control_connectors_list(request: JsonDict = Body(default={})):
        h = connectors_list_handler or handle_connectors_list
        result = h(request)
    ...
    async def control_connectors_get(request: JsonDict = Body(...)):
        h = connectors_get_handler or handle_connectors_get
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_connectors_create(request: JsonDict = Body(...)):
        h = connectors_create_handler or handle_connectors_create
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_connectors_update(request: JsonDict = Body(...)):
        h = connectors_update_handler or handle_connectors_update
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_connectors_delete(request: JsonDict = Body(...)):
        h = connectors_delete_handler or handle_connectors_delete
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_connectors_test(request: JsonDict = Body(...)):
        h = connectors_test_handler or handle_connectors_test
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_oauth_start(request: JsonDict = Body(...)):
        h = oauth_start_handler or handle_oauth_start
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def oauth_callback(code: str = Query(...), state: str = Query(...)):
        h = oauth_callback_handler or handle_oauth_callback
        html = await h(code, state)
        return HTMLResponse(content=html)

    @router.post("/api/control/integrations.oauth.status")
    async def control_oauth_status(request: JsonDict = Body(...)):
        h = oauth_status_handler or handle_oauth_status
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
