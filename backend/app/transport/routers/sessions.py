...
def build_control_sessions_router(
    *,
    sessions_list_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    sessions_resolve_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    sessions_history_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    sessions_send_handler: Callable[[JsonDict, str | None], JsonDict | Awaitable[JsonDict]] | None = None,
    sessions_spawn_handler: Callable[[JsonDict, str | None], JsonDict | Awaitable[JsonDict]] | None = None,
    sessions_status_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    sessions_get_handler: Callable[[JsonDict], JsonDict | Awaitable[JsonDict]] | None = None,
    sessions_patch_handler: Callable[[JsonDict, str | None], JsonDict | Awaitable[JsonDict]] | None = None,
    sessions_reset_handler: Callable[[JsonDict, str | None], JsonDict | Awaitable[JsonDict]] | None = None,
) -> APIRouter:
    router = APIRouter()

    @router.post("/api/control/sessions.list")
    async def control_sessions_list(request: JsonDict = Body(...)):
        h = sessions_list_handler or api_control_sessions_list
        result = h(request)
    ...
    async def control_sessions_resolve(request: JsonDict = Body(...)):
        h = sessions_resolve_handler or api_control_sessions_resolve
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_sessions_history(request: JsonDict = Body(...)):
        h = sessions_history_handler or api_control_sessions_history
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_sessions_send(
        ...
        h = sessions_send_handler or api_control_sessions_send
    ...
    async def control_sessions_spawn(
        ...
        h = sessions_spawn_handler or api_control_sessions_spawn
        result = h(request, idempotency_key_header)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_sessions_status(request: JsonDict = Body(...)):
        h = sessions_status_handler or api_control_sessions_status
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_sessions_get(request: JsonDict = Body(...)):
        h = sessions_get_handler or api_control_sessions_get
        result = h(request)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_sessions_patch(
        ...
        h = sessions_patch_handler or api_control_sessions_patch
        result = h(request, idempotency_key_header)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
    ...
    async def control_sessions_reset(
        ...
        h = sessions_reset_handler or api_control_sessions_reset
        result = h(request, idempotency_key_header)
        awaited = _maybe_await(result)
        return await awaited if awaited is not None else result
